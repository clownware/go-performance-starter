...
- **Amendment 2026-08-19 (#82):** the reaper gained a second, auth-side pass — anonymous GoTrue identities older than the TTL with **no** `public.users` row (failed provisioning, or a sign-in that never reached a UserLoader route) are listed via the admin API and deleted, so orphans no longer linger forever. Both passes need the service-role key; the orphan pass deletes only identities the database positively reports as row-less. Covered by `internal/jobs` table-driven tests and the `ReaperRepo` integration test.
